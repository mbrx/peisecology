...
int main (int argc, char **argv)
{
  ...
  while(peisk_isRunning()) {
    ...
    if(cnt % 25 == 0) {
      ...
      for(i=0;i<nWindows;i++)
	if(windows[i].kind != eUnused) {
	  GtkLabel *clockLabel = GTK_LABEL(gtk_builder_get_object(windows[i].builder,"clockLabel"));
	  if(clockLabel) {
...
void createMainWindow() {
  ...
  snprintf(xmlfilename,sizeof(xmlfilename),"%s/%s/mainwindow.ui",PACKAGE_DATA_DIR,PACKAGE);
  ...
  gtk_widget_modify_font(GTK_WIDGET(gtk_builder_get_object(builder,"editValueTextView")),monofont);
  gtk_widget_modify_font(GTK_WIDGET(gtk_builder_get_object(builder,"currentValueTextView")),monofont);

  ToplevelWindow *tl = insertToplevelWindow(eMainWindow, mainwindow, builder);
  gtk_builder_connect_signals(builder,(gpointer) tl);

  GtkTreeStore *tree = createTupleTreeStore(tl,"peistreeview",G_CALLBACK(peistree_selection_changed_cb));
  tl->peisTree = tree;
  repopulateTree(tree); 

  GtkTreeStore *metaTree = createTupleTreeStore(tl,"MetaTupleTreeView",G_CALLBACK(metatree_selection_changed_cb));
  tl->metaTree = metaTree;
  repopulateTree(metaTree); 

  tl->selectedPeisID=0;
  tl->selectedTuple[0]=0;

  initializeTupleValueHistory(tl);
...
void repopulateAllEcologyviews() {
  ...
  for(i=0;i<nWindows;i++)
    if(windows[i].kind != eUnused)
      repopulateEcologyview(&windows[i]); 
}
...
ToplevelWindow *insertToplevelWindow(EToplevelWindowKind kind, GtkWidget *widget, GtkBuilder *builder) {
...
GtkTreeStore *createTupleTreeStore(ToplevelWindow *tl,char *widgetName,GCallback callback) {
  int i;
  GtkTreeStore *store;
  ...
  GtkWidget *treeview, *metaTupleTreeview;
  store=gtk_tree_store_new(PTREE_NCOLUMNS,G_TYPE_INT,G_TYPE_INT,G_TYPE_STRING,G_TYPE_STRING);
  tupleTreeStores[i]=store;

  /* Insert the treestore into main PeisTree widget */
  treeview=GTK_WIDGET(gtk_builder_get_object(tl->builder,widgetName));
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview),GTK_TREE_MODEL(store));

   /* The view now holds a reference.  We can get rid of our own reference */
  g_object_unref (G_OBJECT (store));

  /* Describe how to render the model */
  ...
  /* Setup the selection handler for main tree view */
  GtkTreeSelection *select;  
  select = gtk_tree_view_get_selection (GTK_TREE_VIEW (treeview));
  gtk_tree_selection_set_mode (select, GTK_SELECTION_SINGLE);
  if(callback) {
    g_signal_connect (G_OBJECT (select), "changed",
		      callback, //G_CALLBACK (peistree_selection_changed_cb),
		      tl);    
  }
