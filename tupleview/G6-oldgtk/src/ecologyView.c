...
** Login   <mbl@brox>
** 
** Started on  Thu Mar 13 11:45:20 2008 Mathias Broxvall
** Last update Fri Mar 14 11:22:57 2008 Mathias Broxvall
*/
...
void on_topologyUpdate_clicked (GtkToolButton   *toolbutton,gpointer         user_data) {
  ...
  /* Update the intervall that the topology should be automatically
     updated */
  int i=0;
  GtkComboBox *update_combo=GTK_COMBO_BOX(lookup_widget(mainwindow,"topologyUpdateInterval"));
  if(update_combo) {
...
void repopulateEcologyview(GtkWidget *mainwindow) {
  int i,k,id;;
  ...
  GtkComboBox *combo=GTK_COMBO_BOX(lookup_widget(mainwindow,"ecologyRoutingTableCombo"));
...
void topologyUpdateView(GtkWidget *mainwindow){
  ...
  printf("Re-rendering ecologyview for window %p\n",mainwindow);

  if(topologyIsUpdating) return; /* Avoid recursion */
  progress = GTK_PROGRESS_BAR(lookup_widget(mainwindow,"ecologyProgressBar"));
  
  cb = GTK_TOGGLE_TOOL_BUTTON(lookup_widget(mainwindow,"ecologyDetailedNodes"));
  useSimpleNodes = !gtk_toggle_tool_button_get_active(cb);
  cb = GTK_TOGGLE_TOOL_BUTTON(lookup_widget(mainwindow,"ecologyShowSubscriptions"));
  showSubscriptions = gtk_toggle_tool_button_get_active(cb);
  cb = GTK_TOGGLE_TOOL_BUTTON(lookup_widget(mainwindow,"ecologyShowConnections"));
  showConnections = gtk_toggle_tool_button_get_active(cb);
  cb = GTK_TOGGLE_TOOL_BUTTON(lookup_widget(mainwindow,"ecologyShowTraffic"));
  showTraffic = gtk_toggle_tool_button_get_active(cb);
  cb = GTK_TOGGLE_TOOL_BUTTON(lookup_widget(mainwindow,"ecologyClusterComponents"));
  clusterComponents = gtk_toggle_tool_button_get_active(cb);

  GtkComboBox *combo=GTK_COMBO_BOX(lookup_widget(mainwindow,"renderingEngine"));
  if(combo) {
    i=gtk_combo_box_get_active(combo);
    switch(i) {
  ...
  /* Draw traffic arrows */
  if(showTraffic || showConnections) {
    GtkSpinButton *thresholdButton = GTK_SPIN_BUTTON(lookup_widget(mainwindow,"ecologyTrafficThreshold"));   
  ...
  GtkComboBox *ecologyRoutingTable=GTK_COMBO_BOX(lookup_widget(mainwindow,"ecologyRoutingTableCombo"));
  ...
  if(currDotFileSize != prevDotFileSize || gPreferences.topologyUpdateInterval == 0){
	...
	renderer="circo";
      ...
      //cancelButton = GTK_TOOL_BUTTON(lookup_widget(mainwindow,"topologyCancel")); 
                                        ...
                                        gpointer         user_data)
{
  ...
  topologyDrawingArea = lookup_widget(mainwindow,"topologyDrawingArea");
  ...
  if(pixbuf == NULL || topologyImageIsNew || topologyImageZoom != zoom) {
	...
	topologyUpdateView(mainwindow); return;
      }
...
void on_topologyZoomIn_clicked              (GtkToolButton   *toolbutton,
                                        gpointer         user_data)
{
  ToplevelWindow *tl = findToplevelWindow(GTK_WIDGET(toolbutton));
  GtkWidget *mainwindow = tl->window;
...
void on_topologyCancel_clicked (GtkToolButton   *toolbutton,gpointer         user_data) {
  ToplevelWindow *tl = findToplevelWindow(GTK_WIDGET(toolbutton));
  GtkWidget *mainwindow = tl->window;
