...
# -allow font to be specified using -f option as opposed to hardcoded FreeSans.
  ...
  else
  {
    ...
    if (! (-e $file_out))
    {
      print "\nImage $file_out not created.\n" ;
      if ((! (-e "pl.exe")) && (! (-e "ploticus")))
...
# $file_pl_err  = $file . ".err" ;
  # MB 120222
  print "Output: Image files $file_out";
...
#    EscapeShellArg($file_out) . " " . EscapeShellArg($file_script) . " -tightcrop -xml_encoding UTF-8" ;
#  print "$cmd\n";
#  system ($cmd) ;
  ...
  $cmd = EscapeShellArg($pl) . " $map -" . $fmt . " -o " .
    EscapeShellArg($file_out) . " " . EscapeShellArg($file_script) . " -tightcrop -font " . EscapeShellArg($font_file);
  print "$cmd\n";
  system ($cmd) ;
