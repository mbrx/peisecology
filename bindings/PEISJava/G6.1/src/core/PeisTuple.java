...
public class PeisTuple extends Structure implements Comparable<PeisTuple> {
	...
	*/
	...
	public Date getTsUser() {
		return PeisJavaUtilities.getJavaDateFromPeisDate(ts_user[0], ts_user[1]);
	}
	
	/**
	 * Sets the user time stamp of the tuple
	 * @param time the new time stamp
	 * @return the old time stamp
	 */
	public Date setTsUser(Date time) {
		return PeisJavaUtilities.setPeisDateFromJavaDate(time, ts_user);
	}
